...
const computeWake = (coords, speeds) => {
  ...
  let starboard = [];
  let alphas = [];
  for (let i = 1; i < coords.length; i += 5) {
    ...
    let spread =
      Phaser.Math.Easing.Cubic.Out(i / coords.length) * LIFE_SPREAD +
      BASE_SPREAD;
...
class Sailing extends Scene {
  ...
  create() {
    ...
    hull.body.postUpdate = function () {
      ...
      deck.setPosition(this.center.x + 1, this.center.y - 1.5);
      deck.setAngle(this.rotation);
      speeds.unshift(this.speed / this.maxSpeed);
      speeds.pop();
      coords.unshift({ x: this.center.x + 1, y: this.center.y });
      coords.pop();
